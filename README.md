...
The timely and precise identification of various dental disorders and oral cancer continues to be a major challenge in clinical dentistry. Current diagnostic methods, such as visual inspection and radiographic imaging, often rely heavily on subjective interpretation and are typically limited to detecting specific types of oral diseases. In response to this need, we propose MultiDentNet, a novel deep learning architecture that enables automated recognition of five prevalent dental issues—dental caries, gingivitis, tooth staining, oral ulcers, and Hypodontia—as well as oral cancer, using intraoral photographs. As far as we are aware, this is the first research effort to simultaneously categorize all five dental pathologies along with oral cancer on such datasets; earlier studies have generally focused on only one or two conditions at a time.
To bridge the gap in multi-disease diagnostic systems, we enhance the DenseNet-121 architecture by incorporating hierarchical self-attention modules that enable the model to effectively capture both fine-grained lesion features and broader contextual information. The system was tested on a carefully compiled dataset comprising 6,791 training, 1,701 validation, and 947 test images for dental conditions, along with a separate oral cancer dataset containing 940 images (490 cancer and 450 non-cancer cases). MultiDentNet demonstrated exceptional performance, achieving an overall accuracy of 99.37%, precision of 99.37%, recall of 99.37%, an F1-score of 0.98, an AUC of 1.00, and a log loss of 0.023 for the dental conditions. For oral cancer detection, the model attained an accuracy of 94.68%, precision of 97.62%, recall of 91.11%, and an F1-score of 0.94. Notably, the model performs well even on less-represented classes, achieving an F1-score of 0.94 for Hypodontia and 0.95 for oral cancer, without requiring additional fine-tuning. Interpretability assessments, including t-SNE visualizations of feature clustering, one-vs-rest ROC and precision-recall curves, and confusion matrix analyses, reveal strong class differentiation and minimal misclassification, underscoring the model’s reliability and potential clinical utility. By combining sophisticated attention mechanisms, class-sensitive loss functions, and thorough interpretability techniques, this study presents a groundbreaking, scalable, and practical AI tool for integrated dental and oral cancer diagnosis. This work lays a solid foundation for future deployment in clinical decision support systems and expansion to a wider range of oral health assessments.
